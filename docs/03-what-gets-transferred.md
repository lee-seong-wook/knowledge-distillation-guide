...
지식 증류는 최종 답만 옮기는 기술이 아니다. 최근 서베이에서는 전달 대상을 크게 `response-based`, `feature-based`, `relation-based`로 정리하는 방식이 널리 쓰인다. 다만 attention-based distillation을 별도 실용 범주로 소개하는 문헌도 있고, feature-based 또는 구조 보존 계열의 하위 방식으로 묶는 문헌도 있다. 이 가이드는 설명 단순화를 위해 후자 쪽 분류를 따른다.
...
teacher의 중간 표현이나 hidden feature를 student가 닮도록 유도하는 방식이다. FitNets가 대표적이다. attention transfer나 self-attention distillation도 큰 틀에서는 이 범주 또는 구조 보존 계열 안에서 설명되는 경우가 많지만, 일부 문헌은 attention-based KD를 별도 범주로 강조하기도 한다. 즉 student가 teacher의 내부 표현 방식을 더 직접적으로 따라 배우게 만드는 접근이다.
...
## attention은 어디에 놓이는가
attention은 직관적으로 이해하기 쉬워서 종종 독립 분류처럼 소개된다. 실제로 일부 서베이와 방법론 정리에서는 attention-based KD를 별도 축으로 다룬다. 반면 다른 정리에서는 teacher의 내부 표현이나 구조를 옮기는 feature-based 계열 안에서 설명하거나, 더 넓게는 structure-preserving 신호의 한 형태로 묶는다.
...
- attention이 자주 따로 소개된다고 해서, 항상 상위 분류로 따로 떼어야 하는 것은 아니다. 문헌에 따라 별도 범주로 두기도 하고 feature/structure 계열의 하위 방식으로 묶기도 한다.
