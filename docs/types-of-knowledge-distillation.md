...
이 저장소는 상위 전달 분류를 `response-based`, `feature-based`, `relation-based`로 통일한다. attention은 중요도가 높아 자주 따로 소개되지만, 큰 틀에서는 feature-based 또는 구조 보존 계열 안에 두는 편이 더 일관적이다.
...
teacher의 중간 표현이나 hidden feature를 student가 닮도록 유도하는 방식이다. FitNets가 대표적이다. attention map이나 self-attention을 맞추는 방식도 보통 이 축 또는 구조 보존 계열 안에서 설명한다.
...
| 학습 구도 | offline, online, self, multi-step | Hinton 2015, Deep Mutual Learning, Born Again, Teacher Assistant | teacher가 언제 어떤 방식으로 student를 가르치는가 |
