...
연도는 이해를 돕기 위해 첫 공개 시점(arXiv 또는 공개본 기준)으로 적었다. 따라서 정식 학회나 저널 출판 연도와 1년 정도 차이 날 수 있다.
...
| 2018 | Born Again Neural Networks | 같은 크기의 student를 반복적으로 재학습해 teacher를 능가할 수 있음을 보여 주었다. |
| 2019 | Improved Knowledge Distillation via Teacher Assistant | teacher와 student 차이가 너무 클 때 중간 단계를 넣는 전략을 제시했다. |
| 2019 | Be Your Own Teacher | 하나의 네트워크 내부에서 지식을 재활용하는 self distillation 흐름을 대표했다. |
