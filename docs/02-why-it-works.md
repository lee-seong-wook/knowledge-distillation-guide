...
## 쉬운 비유
동물 사진 문제에서 정답이 고양이라고만 알려 주는 것과, 고양이일 가능성이 높지만 여우와 개와도 조금 비슷하다고 알려 주는 것은 다르다. 후자는 왜 이 문제가 쉬운지, 혹은 왜 헷갈리는지를 함께 알려 준다. 지식 증류에서 teacher는 이런 입체적인 힌트를 student에게 준다.
...
여기서 중요한 것은 수식 자체보다 의미다. `q_i`는 temperature가 적용된 확률 분포를 뜻하고, `L_total`은 정답 손실과 증류 손실의 조합이다. 지식 증류는 teacher의 분포를 따라가는 학습과 정답을 맞추는 학습을 동시에 수행한다.
