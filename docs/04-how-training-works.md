...
## 쉬운 비유
학원을 떠올리면 이해하기 쉽다. 이미 완성된 스타 강사가 학생을 가르치면 offline 방식에 가깝다. 친구들끼리 서로 설명하며 함께 실력이 오르면 online 방식이다. 스스로 요약 노트를 만들고 자기 자신을 다시 가르치면 self distillation에 가깝다. 그리고 너무 어려운 강사 대신 중간 수준의 조교를 거치는 방식은 teacher assistant에 해당한다.
